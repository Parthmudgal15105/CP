...
void solve()
{
    int n;
    cin >> n;
    // Store array elements and calculate total sum
    ...
    // Check all prefix sums (excluding the full array)
    // If any prefix sum >= total sum, Yasser won't be happy
    long long pref = 0;
    for (int i = 0; i < n - 1; i++)
    {
        pref += a[i];
    ...
    // Check all suffix sums (excluding the full array)
    // If any suffix sum >= total sum, Yasser won't be happy
    long long suff = 0;
    for (int i = n - 1; i > 0; i--)
    {
        suff += a[i];
    ...
    // If no subsegment sum is >= total sum
    // Then Yasser will always be happy
    cout << "YES\n";
}

/*
Today, Yasser and Adel are at the shop buying cupcakes. There are n
 cupcake types, arranged from 1
 to n
 on the shelf, and there are infinitely many of each type. The tastiness of a cupcake of type i
 is an integer ai
...
On the other hand, Adel will choose some segment [l,r]
 (1≤l≤r≤n)
 that does not include all of cupcakes (he can't choose [l,r]=[1,n]
) and buy exactly one cupcake of each of types l,l+1,…,r
.
...
The second line of each test case contains n
 ...
 represents the tastiness of the i
-th type of cupcake.
...
In the second example, Adel will choose the segment [1,2]
 with total tastiness 11
, which is not less than the total tastiness of all cupcakes, which is 10
.

In the third example, Adel can choose the segment [3,3]
 with total tastiness of 5
. Note that Yasser's cupcakes' total tastiness is also 5
, so in that case, the total tastiness of Yasser's cupcakes isn't strictly greater than the total tastiness of Adel's cupcakes.
